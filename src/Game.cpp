...
#include "./Components/ProjectileEmitterCopmnent.h"
#include "Map.h"
...
void Game::Update() {
	// Sleep the exeution until we reach the target frame time in miliseconds 
	int timeToWait = TARGET_FRAME_RATE - (SDL_GetTicks() - ticksLastFrame);

	// Only delay if we are too fast to process his frame 
	if (timeToWait > 0 && timeToWait <= TARGET_FRAME_RATE){
		SDL_Delay(timeToWait);
	}	
	

	// deltaTime is the difference in ticks from the last frame converted into seconds 
    	float deltaTime = (SDL_GetTicks() - ticksLastFrame) / 1000.0f;

	//Clamp the delta time to maximum value
	deltaTime = (deltaTime > 0.05f) ? 0.05f : deltaTime;

	// Sets the new ticks for the current frame to be used in the next pass
	ticksLastFrame = SDL_GetTicks();
	
	/*manager.Update(deltaTime); loops all the entities of the manager calling 
	their update and their entities update is going call component's update */
